...
class PlayingState(MediaState):
	def update_presence(self, media_info):
		...
		if playback:
			...
			position = playback.get("time", 0)  # This is current playback position in seconds

			# Only set timestamps if we have valid data
			if position >= 0 and duration > 0 and duration < 86400:  # Sanity check (under 24 hours)
				start_timestamp = current_time - position  # When the media started playing
